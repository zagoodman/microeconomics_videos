...
% ZG:  However, I'm not sure that relates closely with the model and hence I'll omit it from the introduction for now.
...
The intervention we study, inducing students to watch pre-recorded instructional videos, almost exclusively involves student time \footnote{As with textbooks, instructor time is used if students ask questions about differences between lecture and the videos or textbook.  It can be particularly time consuming if the answer to an exam question differs across lecture, videos or textbook}.  Since the IMVH is the first videobook to our knowledge, ours is the first to examine the effects of access to a video book.  However, students have many alternatives to help them learn the course material, including attending class, reading the textbook, doing homework, and taking quizzes, and there is a large empirical literature examining the effects of each.  For each learning method, a positive correlation between course performance and use of the learning method is not enough to establish causality because of unobserved student characteristics, such as motivation, that are positively correlated with the use of a learning method and class performance.  As a result, we focus on empirical studies that use experiments and quasi-experiments.  
