...
In Table~\ref{tab:spillover_100b}, we estimate the effect of treatment on taking Micro B and the number of classes passed and withdrawn.
...
In Table~\ref{tab:microb_results}, we examine the effects of treatment on video watching and exam scores in Micro B among students who enrolled in Micro B the next quarter.
These comparisons may be biased due to differential selection into Micro B by treatment status.
We address this selection problem using three approaches.

Column (1) reports post-double-selection estimates on the selected sample of Micro B takers as a benchmark.
Column (2) computes generalized Lee bounds \parencite{semenova2025}, which relax the standard monotonicity assumption of \textcite{lee2009} to conditional monotonicity: treatment may increase Micro B enrollment for some students (e.g., those whose Micro A performance improved) while decreasing it for others (e.g., those who spent more time on Micro A with no grade benefit), as long as the direction is consistent within covariate subgroups.
...
Column (3) uses inverse probability weights (IPW) to reweight Micro B takers to resemble the full experimental sample on observable baseline characteristics.
...
In Panel A, we find large effects on video watching: treatment caused students to watch substantially more unique videos and hours of content compared to control students.
In Panel B, we estimate treatment effects on Micro B exam scores.
...
%Students could opt out at any point during the course and so could have seen their treatment status, but it did not affect their opted out after seeing treatment status.
