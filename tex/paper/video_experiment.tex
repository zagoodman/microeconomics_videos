...
% notezg I think part of this paragraph can be removed as the grading scheme is covered earlier in the paper. The part about curving could be moved closer to that portion of the paper. The part about randomizing students is covered elsewhere.
...
% notezg I think we should leave this out, or move it to the appendix. For the purpose of analyzing the experiment, I only look at the administrative data and do not consider the survey data as compliance here was not great. I think if we mention this at all, we could write in the appendix how students were kept informed about their video counts and mention that some students may have "watched" a video twice because they forgot to submit the link the first time.
...
where $\tau$ is the average causal effect of watching videos and $Y_{it}(1)$ and $Y_{it}(0)$ are the potential outcomes (i.e. exam scores) for a student $i$ in year $t$ who does and does not watch videos, respectively. We can observe incentive arm assignment for each student, $Z_{it} \in \{0,1\}$, as well as the observed outcome $Y_{it}(Z=Z_{it})$ and a vector of pretreatment covariates $X_{it}$.

A student's decision to watch videos is endogenous, so we must rely on an exogenous instrument to calculate an unbiased estimate of $\tau$. By randomly assigning a grade incentive that induces significantly greater video watching, the indicator $Z_{it}$ satisfies the instrument validity and relevancy conditions required to estimate $\tau$ using an instrumental variable approach \parencite{ir2015}. We calculate an estimate of $\tau$ using two-stage least squares:
...
\begin{equation}
	Y_{it} = \beta Z_{it} + h(X_{it}) + \epsilon_{it}
\end{equation}

where $Y_{it}$ is an outcome of interest (e.g. videos watched or test scores) for student $i$ in year $t$, $\beta$ is the ITT parameter of interest, $h()$ is a generic function through which $X_{it}$, a vector of pretreatment controls, affects $Y_{it}$, and $\epsilon_{it}$ is an unobserved residual assumed to be mean zero.

For $\beta$ to take a causal interpretation, we make three assumptions. First, we assume treatment status is unconfounded by observable and unobservable covariates, i.e. $Z_{it} \perp \!\!\! \perp \Big(Y_{it}(Z=1), Y_{it}(Z=0) \Big| X_{it}\Big)$. Unconfoundedness guarantees that the ITT estimate is not biased by imbalances in covariates across treatment arms. Although random treatment assignment ensures that in expectation treatment arms are balanced thereby satisfying unconfoundedness, in small samples it is possible to have imbalance that could bias $\hat{\beta}$ away from the true ITT. We check balance on observable pretreatment variables in table \ref{balancetable} and find XX. Additionally, in our specifications, we control for slight imbalances in observable covariates and find that our ITT estimates are nearly unaffected. Of course, we cannot check for balance or control in our specifications for imbalances on unobserved covariates, hence we must \textit{assume} unconfoundedness. We additionally assume \textit{excludability} and \textit{non-interference} or \textit{SUTVA}, which are the same assumptions from the LATE estimation described earlier in this section.

% Neyman test of zero avg causal effect

In addition to the partial linear specifications described above, we also estimate the ITT and associated confidence intervals using Neyman's \citeyear{neyman1923} repeated sampling framework, considering each pair (block) a completely randomized experiment and combining the results. We begin by estimating the point estimate of the ITT as the difference in means between the \textit{Incentive} and \textit{Control} arms, which is equivalent to the average difference in means across all pairs:

\begin{equation} \label{itt_spec}
	\hat{\tau} = \bar{\tau}_I - \bar{\tau}_C = \frac{1}{J}\sum_{j=1}^{J} \bar{y}_{j,I} - \bar{y}_{j,C}
\end{equation}

where $\hat{\tau}$ is the point estimate of the ITT, $J$ is the number of pairs in the sample, and other notation follows from above. The estimate $\hat{\tau}$ is an unbiased estimate of the true population ITT $\tau$ given unconfoundedness, excludability, and non-interference.

We will test the null hypothesis that being treated has zero effect against the alternative that being treated has a nonzero effect on exam scores. Following Neyman's repeated sampling approach, the estimated standard error of $\hat{\tau}$ (\cite{imai2008}, \cite{ir2015}, \cite{ai2017}) is:
...
Although this estimator is conservative if viewed as the estimate of the sample ITT standard error, it is an unbiased estimator if viewed from the perspective of pairs drawn randomly from a much larger superpopulation \parencite{ai2017}.
