...
We present our estimates of spillover effects in the subsequent term in Tables~\ref{tab:spillover_100b} and~\ref{tab:microb_results}.
...
In Panel C of Table~\ref{tab:spillover_100b}, we estimate the effect of treatment on taking Micro B and the number of classes passed and withdrawn.
...
In Panels A and B of Table~\ref{tab:spillover_100b}, we examine the effects of treatment on video watching and exam scores in Micro B among students who choose to take Micro B the next quarter.
...
We address this selection problem in Table~\ref{tab:microb_results} using two approaches.

First, we compute generalized Lee bounds \parencite{semenova2025}, which relax the standard monotonicity assumption of \textcite{lee2009} to conditional monotonicity: treatment may increase Micro B enrollment for some students (e.g., those whose Micro A performance improved) while decreasing it for others (e.g., those who spent more time on Micro A with no grade benefit), as long as the direction is consistent within covariate subgroups.
This is precisely the structure suggested by our data; treatment improved Micro A exam scores, which is likely to increase the probability of taking Micro B, but treatment also increased time spent on Micro A with no effect on course grades, which seems likely to reduce the probability of taking Micro B.
Following \textcite{heiler2024}, we condition on baseline covariates to tighten the bounds.

Second, we use inverse probability weights (IPW) to reweight Micro B takers to resemble the full experimental sample on observable baseline characteristics.
The IPW results should be interpreted with caution, as they correct for selection on observables but cannot address unobserved demoralization or other latent selection channels.
...
In Panel A of Table~\ref{tab:spillover_100b}, we estimate the effect of treatment on videos watched during the subsequent term, for those who took Micro B.
...
\clearpage
\input{../tables/table9_microb.tex}
